...
One of the nice things about MongoDB Vector Search is its seamless integration with the MongoDB ecosystem. For instance, to do a vector search, we use an aggregation pipeline stage, and after searching, we can project, limit our data, etc. But sometimes, we want to filter _before_ running the semantic search. For that, we can use the optional `filter` property in `$vectorSearch`.
