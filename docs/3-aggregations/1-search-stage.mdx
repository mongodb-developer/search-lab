...
You can construct MongoDB Search queries with the `$search` aggregation pipeline stage.
...
In this section, we'll build an aggregation pipeline with the `$search` stage which performs full-text search using the MongoDB Search index you just created.
